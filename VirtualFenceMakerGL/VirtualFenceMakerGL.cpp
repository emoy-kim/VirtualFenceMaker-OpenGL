...
void ObjectGL::prepareTexture(const int& n_bytes_per_vertex, const string& texture_file_name)
{
...
void ObjectGL::setObject(GLenum draw_mode, const glm::vec3& color, const vector<glm::vec3>& vertices)
{
	DrawMode = draw_mode;
...
VirtualFenceMakerGL *VirtualFenceMakerGL::Renderer = nullptr;
VirtualFenceMakerGL::VirtualFenceMakerGL(const float& actual_width, const float& actual_height) :
	RenderWindow( nullptr ), ClickedPoint( -1, -1 ), DrawFenceOnGroundOnly( false ),
...
void VirtualFenceMakerGL::printOpenGLInformation() const
{
	cout << "****************************************************************" << endl;
...
)
{
	...
	MainCamera.ToWorldCoordinate = 
		...
		glm::inverse( MainCamera.PanningToCamera ) * glm::inverse( MainCamera.TiltingToCamera );
	...
	MainCamera.ViewMatrix = glm::lookAt( MainCamera.CameraPosition, glm::vec3(viewing_point), glm::vec3(up_vector) );
...
void VirtualFenceMakerGL::captureFenceMask()
{
	FenceMask.create( MainCamera.Height, MainCamera.Width, CV_8UC1 );
	glPixelStorei( GL_PACK_ALIGNMENT, FenceMask.step & 3 ? 1 : 4 );
	glReadBuffer( GL_BACK );
	glReadPixels( 0, 0, FenceMask.cols, FenceMask.rows, GL_RED, GL_UNSIGNED_BYTE, FenceMask.data );
	flip( FenceMask, FenceMask, 0 );

	FenceMask.setTo( 0, FenceMask == 255 );
	FenceMask.setTo( 255, FenceMask != 0 );
	imshow( "Fence Mask", FenceMask );
}
...
void VirtualFenceMakerGL::updateFenceHeight(const double& mouse_wheel_y_offset)
{
...
void VirtualFenceMakerGL::updateFenceRadius(const double& mouse_wheel_y_offset)
{
	if (mouse_wheel_y_offset >= 0.0) {
		FenceRadius += 5.0f;
...
void VirtualFenceMakerGL::setGroundObject()
{
	...
	const vector<glm::vec2> ground_textures = {
		glm::vec2(0.0f, 1.0f),
		glm::vec2(1.0f, 1.0f),
		glm::vec2(0.0f, 0.0f),
		glm::vec2(0.0f, 0.0f),
	...
	Ground.setObject( GL_TRIANGLES, ground_color, ground_vertices, ground_textures, "ground.jpg" );
}
...
bool VirtualFenceMakerGL::getWorldPoint(glm::vec3& fence_center, const float& height_from_ground) const
{
...
void VirtualFenceMakerGL::drawGround()
{
	...
	glUniform3fv( GroundShader.ColorLocation, 1, glm::value_ptr( Ground.Colors ) );
...
void VirtualFenceMakerGL::drawFenceAtCenter(const glm::vec3& center)
{
	...
	glUniform3fv( FenceShader.ColorLocation, 1, glm::value_ptr( Fence.Colors ) );
